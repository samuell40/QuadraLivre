<template>
    <div class="placar">
        ...
        <div class="box">
            <p>Substituições</p>
            <div class="controls">
                <button @click="abrirModalRemoverJogador"
                ...
                <button @click="abrirModalSubstituicao" :disabled="!temporizadorAtivo">
                    +
                </button>
        ...
        <div v-if="substituicaoModal" class="modal-overlay" @click.self="fecharModalSubstituicao">
            <div class="modal-content">
                ...
                <div v-if="substituicoesPendentes.length" class="coluna coluna-substituicoes">
                    ...
                    <div v-for="(s, index) in substituicoesPendentes" :key="index"
                        class="jogador-card substituicao-card">
                        <div class="jogador-info">
                ...
                <div class="botoes">
                    ...
                    <button class="btn-save1" v-if="substituicoesPendentes.length > 0"
                        @click="confirmarTodasSubstituicoes">
        ...
        <div v-if="modalRemoverAberto" class="modal-overlay" @click.self="fecharModalRemover">
            <div class="modal-content modal-remover">
                ...
                <div class="botoes">
                    <button class="btn-save1" :disabled="!jogadoresSelecionados.length"
                        @click="confirmarRemocaoJogadores">
    ...
    methods: {
        ...
        async alterarEventoJogador(jogador, acao) {
            ...
            if (this.tipoEvento === 'amarelo') {
                ...
                // FUTSAL: 2 amarelos = vermelho automático
                if (jogador.cartoesAmarelos === 2 && jogador.cartoesVermelhos === 0) {
                    jogador.cartoesVermelhos = 1
                    this.localTime.cartaovermelho += 1
                    payload.cartoesVermelhos = 1

                    Swal.fire(
                        'Cartão Vermelho!',
                        'Jogador expulso por 2 cartões amarelos',
                        'warning'
                    )

                    this.iniciarPunicaoFutsal(jogador.id)
                }
            }
            if (this.tipoEvento === 'vermelho') {
                if (jogador.cartoesVermelhos + incremento < 0) return

                // evita vermelho duplicado
                if (incremento > 0 && jogador.cartoesVermelhos > 0) return
        ...
        iniciarPunicaoFutsal(jogadorId) {
            // remove jogador de campo
            this.jogadoresEmCampo = this.jogadoresEmCampo.filter(
                j => j.id !== jogadorId
            )
            ...
            const timeout = setTimeout(() => {
                Swal.fire(
                    'Punição encerrada',
                    'Time pode voltar com jogador',
                    'info'
                )

                // opcional: abrir direto o modal de substituição
                this.abrirModalSubstituicao()

            }, 120000) // 2 minutos

            this.punicoes.push({ jogadorId, timeout })
        },
